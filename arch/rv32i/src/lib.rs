...
global_asm!(
    "
          ...
          _start_trap:
            ...
            //
            // No registers other than s0, s1 and the mscratch CSR are to be
            // clobbered before contiuing execution at the address loaded into
            ...
            // trap handler.
            //
            // We explicitly use callee-saved over temporary registers, as this
            // makes it easier to call other functions as part of the trap,
            // handler, such as to disable interrupts. This global trap handler
            // saves the previous values of these registers anyways, and places
            // new, important values into them (the previous mscratch value and
            // the trap handler address). These values are likely desired to
            // not be clobbered by further function calls in the trap handler.
            //
            // When a custom trap handler stack is registered in `mscratch`, the
