...
impl kernel::syscall::UserspaceKernelBoundary for SysCall {
    ...
    unsafe fn switch_to_process(
        ...
        asm!("
          ...
          // the mscratch CSR so we can retrieve it after returning to
          ...
          lw    x8,  7*4(a0)        // s0,fp
          lw    x9,  8*4(a0)        // s1
          // ------------------------> a0, do last since we overwrite our pointer
          lw   x11, 10*4(a0)        // a1
          lw   x12, 11*4(a0)        // a2
          lw   x13, 12*4(a0)        // a3
          lw   x14, 13*4(a0)        // a4
        ...
        100: // _start_app_trap
          ...
          // an app. We don't know _why_ we got a trap, it could be fro an
